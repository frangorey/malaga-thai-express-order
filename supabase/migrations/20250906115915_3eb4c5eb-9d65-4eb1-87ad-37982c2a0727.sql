...
-- Clear existing products and insert new structured data
TRUNCATE TABLE public.products RESTART IDENTITY CASCADE;

-- Insert arroz frito products with Classic sauce
...
-- Insert arroz frito products with Original sauce
...
-- Insert arroz frito products with Teriyaki sauce
...
-- Insert arroz frito products with Curry Amarillo
...
-- Insert arroz frito products with Curry Verde
...
-- Insert arroz frito products with Curry Rojo
...
-- Now create the relationships in junction tables
-- First, create relationships between products and sauces
WITH sauce_products AS (
  SELECT p.id as product_id, s.id as sauce_id
  FROM products p 
...
SELECT product_id, sauce_id FROM sauce_products;

-- Create relationships between products and ingredients based on product names
WITH ingredient_mapping AS (
  SELECT p.id as product_id, i.id as ingredient_id
  FROM products p
...
SELECT DISTINCT product_id, ingredient_id FROM ingredient_mapping;
