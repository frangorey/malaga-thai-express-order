...
export type NoodleType = "Anchos" | "Finos" | "Glass" | "Udon";

interface NoodleCustomizerDrawerProps {
  open: boolean;
  onClose: () => void;
...
interface SauceOption {
  id: string;
  name: string;
  dbName: string; // pattern in DB product name
  color: string;
}
...
export const NoodleCustomizerDrawer = ({ open, onClose, onAddToCart, noodleType, editingItem }: NoodleCustomizerDrawerProps) => {
  ...
  const sauces: SauceOption[] = [
    { id: "classic", name: t("sauce_classic"), dbName: "classic", color: "bg-amber-500" },
    { id: "original", name: t("sauce_original"), dbName: "original", color: "bg-green-600" },
    { id: "teriyaki", name: "Teriyaki", dbName: "teriyaki", color: "bg-amber-700" },
  ];
  ...
  const handleAddToCart = () => {
    ...
    const customizationData: CustomizationData = {
      customizerType: 'noodle',
      drawerVariant: noodleType,
      selections: {
      ...
      <DrawerContent className="max-h-[90dvh] bg-background">
        <DrawerHeader className="flex items-center justify-between pb-2 border-b border-border">
          <DrawerTitle className="text-lg font-bold">
            🍜 Tallarines {NOODLE_LABELS[noodleType]}
          </DrawerTitle>
        ...
        <div className="flex items-center gap-1 px-4 py-3 overflow-x-auto">
          {STEP_ORDER.map((step, i) => {
            ...
            return (
              <button
                key={step}
                onClick={() => { if (isDone) setCurrentStep(step); }}
                className={cn(
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4" style={{ maxHeight: "60dvh" }}>
              ...
              {proteins.map((p) => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedProtein === p.id ? "border-primary bg-primary/10" : "border-border hover:border-primary/50"
                  )}
                >
          ...
          {currentStep === "sauce" && (
            <div className="space-y-2 pt-2">
              <p className="text-sm text-muted-foreground mb-3">Elige tu salsa</p>
              {sauces.map((s) => (
                <button
                  key={s.id}
                  ...
                  className={cn(
                    ...
                    selectedSauce === s.id ? "border-primary bg-primary/10" : "border-border hover:border-primary/50"
                  )}
                >
          ...
          {currentStep === "vegetables" && (
            <div className="pt-2">
                ...
                {vegetables.map((v) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedVegetables.includes(v.id) ? "border-primary bg-primary/10" : "border-border hover:border-primary/50"
                    )}
                  >
                    {selectedVegetables.includes(v.id) && <Check className="w-4 h-4 text-primary mb-1" />}
              ...
              <div className="flex gap-2 mt-4">
                <Button variant="outline" size="sm" onClick={goBack} className="flex-1">← {t("step_sauce")}</Button>
                <Button variant="default" size="sm" onClick={goNext} className="flex-1">{t("step_extras")} →</Button>
              </div>
            </div>
          )}
          ...
          {currentStep === "extras" && (
            <div className="pt-2 space-y-4">
              <div>
                <h4 className="text-sm font-semibold mb-2">🥫 Salsas extra</h4>
                <div className="grid grid-cols-2 gap-2">
                  {sauceExtras.map((e) => (
                    <button
                      key={e.id}
                      ...
                      className={cn(
                        ...
                        selectedExtras.includes(e.id) ? "border-primary bg-primary/10" : "border-border hover:border-primary/50"
                      )}
                    >
              ...
              <div className="flex gap-2 mt-2">
                <Button variant="outline" size="sm" onClick={goBack} className="flex-1">← {t("step_vegetables")}</Button>
                <Button variant="default" size="sm" onClick={goNext} className="flex-1">{t("order_summary")} →</Button>
              </div>
            </div>
          )}
            ...
            <div className="pt-2 space-y-3">
              <div className="rounded-xl border border-border p-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">Tallarines {NOODLE_LABELS[noodleType]}</span>
                </div>
                <div className="flex justify-between">
