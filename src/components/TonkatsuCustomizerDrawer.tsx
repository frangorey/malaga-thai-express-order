...
type SauceId = "sweet_sour" | "bbq";

const TOTAL_PRICE = 13.5;
const SLUG = "tonkatsu";

export const TonkatsuCustomizerDrawer = ({ open, onClose, onAddToCart, editingItem }: Props) => {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    if (!open) return;
    if (editingItem) {
      if (editingItem.customizationData?.customizerType !== 'tonkatsu') {
        resetSelections();
        return;
      }
  ...
  const garnishes: { id: GarnishId; name: string; emoji: string; matchToken: string }[] = [
    { id: "fried_rice", name: t("tonkatsu_garnish_fried_rice"), emoji: "🍚", matchToken: "Arroz Frito" },
    { id: "fried_noodles", name: t("tonkatsu_garnish_fried_noodles"), emoji: "🍜", matchToken: "Fideos Fritos" },
  ];

  const sauces: { id: SauceId; name: string; matchToken: string; ring: string }[] = [
    { id: "sweet_sour", name: t("tonkatsu_sauce_sweet_sour"), matchToken: "Agridulce", ring: "border-amber-500/70" },
    { id: "bbq", name: t("tonkatsu_sauce_bbq"), matchToken: "Barbacoa", ring: "border-red-500/70" },
  ];
    ...
    <Drawer open={open} onOpenChange={(o) => !o && handleClose()}>
      <DrawerContent className="max-h-[90dvh] bg-background">
        <DrawerHeader className="flex items-center justify-between pb-2 border-b border-border">
          <DrawerTitle className="text-lg font-bold">🍱 Tonkatsu</DrawerTitle>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-24 pt-4" style={{ maxHeight: "60dvh" }}>
          ...
          {!isLoading && !isError && templateProducts.length > 0 && (
            <>
              <p className="text-sm text-muted-foreground mb-3">{t("tonkatsu_choose_garnish")}</p>
              ...
              {selGarnish && (
                <>
                  <p className="text-sm text-muted-foreground mt-5 mb-3">{t("tonkatsu_choose_sauce")}</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {sauces.map((s) => (
                      <button
                        key={s.id}
              ...
              {selGarnish && selSauce && (
                <div className="mt-5 p-4 rounded-xl border border-border bg-muted/40 space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">Tonkatsu</span>
                  </div>
                  <div className="flex justify-between text-sm">
